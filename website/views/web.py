...
def get_homepage_ajax(request, task_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            # return JSON/Dict like during Desktop Upload
            html = render_to_string('website/index_ajax.html', context)
...
def handle_www_profile_upload_ajax(request, task_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            mons = Monster.objects.filter(id__in=[
                                          mon['id'] for mon in context['comparison']['monsters']]).prefetch_related('base_monster')
            runes = Rune.objects.filter(id__in=[
                                        rune['id'] for rune in context['comparison']['runes']]).prefetch_related('rune_set')
            ...
            html = render_to_string(
                'website/upload/upload_ranking.html', context)
            return HttpResponse(html)
...
def get_runes_ajax(request, task_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['best_runes'] = Rune.objects.filter(id__in=context['best_runes_ids']).prefetch_related(
                'rune_set', 'equipped_runes', 'equipped_runes__base_monster', 'equipped_runes_rta', 'equipped_runes_rta__base_monster').order_by('-efficiency')
            context['fastest_runes'] = Rune.objects.filter(id__in=context['fastest_runes_ids']).prefetch_related(
                'rune_set', 'equipped_runes', 'equipped_runes__base_monster', 'equipped_runes_rta', 'equipped_runes_rta__base_monster').order_by('-sub_speed')

            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/runes/rune_index_ajax.html', context)
            return HttpResponse(html)
...
def get_rune_by_id_ajax(request, task_id, arg_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['rune'] = get_object_or_404(Rune.objects.prefetch_related('rune_set', 'equipped_runes', 'equipped_runes__base_monster',
                                                                              'equipped_runes__runes', 'equipped_runes__runes__rune_set', 'equipped_runes_rta', 'equipped_runes_rta__base_monster'), id=arg_id)
            context['rta_monster'] = context['rune'].equipped_runes_rta.first()
            context['similar_runes'] = Rune.objects.filter(id__in=context['similar_ids']).prefetch_related(
                'rune_set', 'equipped_runes', 'equipped_runes__base_monster', 'equipped_runes_rta', 'equipped_runes_rta__base_monster').order_by('-efficiency')

            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/runes/rune_by_id_ajax.html', context)
            return HttpResponse(html)
...
def get_artifacts_ajax(request, task_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['best_artifacts'] = Artifact.objects.filter(id__in=context['best_artifacts_ids']).prefetch_related(
                'equipped_artifacts', 'equipped_artifacts__base_monster', 'equipped_artifacts_rta', 'equipped_artifacts_rta__base_monster').order_by('-efficiency')

            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/artifacts/artifact_index_ajax.html', context)
            return HttpResponse(html)
...
def get_artifact_by_id_ajax(request, task_id, arg_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['artifact'] = get_object_or_404(Artifact.objects.prefetch_related(
                'equipped_artifacts', 'equipped_artifacts__base_monster', 'equipped_artifacts_rta', 'equipped_artifacts_rta__base_monster'), id=arg_id)
            context['similar_artifacts'] = Artifact.objects.filter(id__in=context['similar_ids']).prefetch_related(
                'equipped_artifacts', 'equipped_artifacts__base_monster', 'equipped_artifacts_rta', 'equipped_artifacts_rta__base_monster').order_by('-efficiency')

            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/artifacts/artifact_by_id_ajax.html', context)
            return HttpResponse(html)
...
def get_monsters_ajax(request, task_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['best_monsters'] = Monster.objects.filter(id__in=context['best_monsters_ids']).prefetch_related(
                'base_monster', 'runes', 'runes__rune_set', 'runes_rta', 'runes_rta__rune_set', 'artifacts', 'artifacts_rta').order_by('-avg_eff')
            context['fastest_monsters'] = Monster.objects.filter(id__in=context['fastest_monsters_ids']).prefetch_related(
                'base_monster', 'runes', 'runes__rune_set', 'runes_rta', 'runes_rta__rune_set', 'artifacts', 'artifacts_rta').order_by('-speed')

            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/monsters/monster_index_ajax.html', context)
            return HttpResponse(html)
...
def get_monster_by_id_ajax(request, task_id, arg_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['monster'] = get_object_or_404(Monster.objects.prefetch_related('base_monster', 'base_monster__family', 'runes', 'runes__rune_set', 'runes_rta', 'runes_rta__rune_set', 'artifacts', 'artifacts_rta', 'siege_defense_monsters', 'dungeon_monsters', 'raid_monster_fl_1', 'raid_monster_fl_2', 'raid_monster_fl_3', 'raid_monster_fl_4',
                                                                                    'raid_monster_bl_1', 'raid_monster_bl_2', 'raid_monster_bl_3', 'raid_monster_bl_4', 'raid_monster_leader', 'rift_monster_fl_1', 'rift_monster_fl_2', 'rift_monster_fl_3', 'rift_monster_fl_4', 'rift_monster_bl_1', 'rift_monster_bl_2', 'rift_monster_bl_3', 'rift_monster_bl_4', 'rift_monster_leader', 'dimhole_monsters'), id=arg_id)

            context['similar_monsters'] = Monster.objects.filter(id__in=context['similar_ids']).prefetch_related(
                'base_monster', 'base_monster__family', 'runes', 'runes__rune_set', 'runes_rta', 'runes_rta__rune_set', 'artifacts', 'artifacts_rta')
            context['rta_similar'] = Monster.objects.filter(id__in=context['rta_similar_ids']).prefetch_related(
                'base_monster', 'base_monster__family', 'runes', 'runes__rune_set', 'runes_rta', 'runes_rta__rune_set', 'artifacts', 'artifacts_rta')
            context['records'] = get_monster_records(context['monster'])

            html = render_to_string(
                'website/monsters/monster_by_id_ajax.html', context)
            return HttpResponse(html)
...
def get_decks_ajax(request, task_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['decks'] = Deck.objects.filter(id__in=context['decks_ids']).prefetch_related(
                'monsters', 'monsters__base_monster', 'monsters__base_monster__family', 'leader', 'leader__base_monster', 'leader__base_monster__family').order_by('-team_runes_eff')

            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/decks/deck_index_ajax.html', context)
            return HttpResponse(html)
...
def get_deck_by_id(request, arg_id):
    ...
    deck = get_object_or_404(Deck.objects.prefetch_related('monsters', 'monsters__base_monster', 'monsters__runes__rune_set',
                                                           'monsters__runes__equipped_runes', 'monsters__runes__equipped_runes__base_monster', 'monsters__base_monster__family'), id=arg_id)
...
def get_deck_by_id_ajax(request, task_id, arg_id):
    if request.is_ajax():
        ...
        if data.ready():
            deck = get_object_or_404(Deck.objects.prefetch_related('monsters', 'monsters__base_monster', 'monsters__runes__rune_set',
                                                                   'monsters__runes__equipped_runes', 'monsters__runes__equipped_runes__base_monster', 'monsters__base_monster__family'), id=arg_id)
            decks = Deck.objects.all().order_by('place').prefetch_related(
                'monsters', 'monsters__base_monster', 'leader', 'leader__base_monster')
            ...
            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/decks/deck_by_id_ajax.html', context)
            return HttpResponse(html)
...
def get_siege_records_ajax(request, task_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['best_records'] = SiegeRecord.objects.filter(id__in=context['records_ids'], wizard__guild__isnull=False).prefetch_related('monsters', 'monsters__base_monster', 'wizard', 'wizard__guild', 'leader', 'leader__base_monster', 'monsters__base_monster__family').annotate(
                sorting_val=Sum(F('wizard__guild__siege_ranking') / 1000 * F('ratio') / 100 * (F('win') / 100.0), output_field=FloatField())).order_by('-sorting_val')[:context['best_amount']]
            ...
            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/siege/siege_index_ajax.html', context)
            return HttpResponse(html)
...
def get_dungeons(request):
    dungeons = DungeonRun.objects.values('dungeon', 'stage', 'win').annotate(avg_time=Avg(
        'clear_time')).annotate(quantity=Count('id')).order_by('dungeon', '-stage', '-win')
    rift_dungeons = RiftDungeonRun.objects.values('dungeon', 'win').annotate(avg_time=Avg(
        'clear_time')).annotate(quantity=Count('dungeon')).order_by('dungeon', '-win')
    ...
    raid_dungeons = RaidDungeonRun.objects.values('stage', 'win').annotate(
        avg_time=Avg('clear_time')).annotate(quantity=Count('stage')).order_by('-stage')
    raid_dungeon_name = 'Rift of Worlds'

    records = dict()
    for dungeon_base in dungeons_base:
        stages = dict()
        max_stage = 12 if DungeonRun().get_dungeon_id(
            dungeon_base) in [6001, 8001, 9001] else 10
    ...
    for rift_dungeon in rift_dungeons:
        dungeon_name = RiftDungeonRun().get_dungeon_name(
            rift_dungeon['dungeon'])
        if not dungeon_name:
            continue
        dungeon_stage = "B1"
    ...
    if request.is_ajax():
        if name.lower() == 'rift of worlds':
            ...
            if data.ready():
                ...
                mons = Monster.objects.filter(id__in=[monster_id for record in context['records_personal'] for monster_id in (
                    record['frontline'] + record['backline']) if monster_id is not None]).prefetch_related('base_monster')

                for mon in mons:
                    for record in context['records_personal']:
                        if mon.id in record['frontline']:
                            record['frontline'][record['frontline'].index(
                                mon.id)] = mon
                        elif mon.id in record['backline']:
                            record['backline'][record['backline'].index(
                                mon.id)] = mon
                ...
                # return JSON/Dict like during Desktop Upload
                html = render_to_string(
                    'website/dungeons/raid_dungeon_by_stage_ajax.html', context)
                return HttpResponse(html)
        else:
            ...
            if data.ready():
                ...
                mons = Monster.objects.filter(id__in=[monster_id for record in context['records_personal']
                                                      for monster_id in record['comp']]).prefetch_related('base_monster')
                ...
                # return JSON/Dict like during Desktop Upload
                html = render_to_string(
                    'website/dungeons/dungeon_by_stage_ajax.html', context)
                return HttpResponse(html)
...
def get_rift_dungeon_by_stage_ajax(request, task_id, name):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/dungeons/rift_dungeon_by_stage_ajax.html', context)
            return HttpResponse(html)
...
def get_dimension_hole_ajax(request, task_id):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/dimhole/dimhole_index_ajax.html', context)
            return HttpResponse(html)
...
def get_homunculus(request):
    homunculuses_base = MonsterBase.objects.filter(
        name__contains='Homunculus', awaken=True)
    homunculuses = WizardHomunculus.objects.all()

    cards = list()
    for homunculus_base in homunculuses_base:
        builds = homunculuses.filter(homunculus__base_monster=homunculus_base).values(
            'build').annotate(Count('id')).order_by()
        cards.append({
...
def get_homunculus_base_ajax(request, task_id, base):
    if request.is_ajax():
        ...
        if data.ready():
            ...
            context['records'] = WizardHomunculus.objects.filter(id__in=context['records_ids']).prefetch_related(
                'homunculus__base_monster').order_by('-homunculus__avg_eff')
            context['skills'] = HomunculusSkill.objects.filter(
                id__in=context['skills_ids'])

            # return JSON/Dict like during Desktop Upload
            html = render_to_string(
                'website/homunculus/homunculus_base_ajax.html', context)
            return HttpResponse(html)
...
def get_object_for_card(request, obj_type, obj_id):
    if obj_type == 'mon':
        html = render_to_string('website/monsters/monster_card_compressed.html', {'monster': Monster.objects.get(
            id=obj_id), 'deep': True, })  # return JSON/Dict like during Desktop Upload
    elif obj_type == 'rune':
        html = render_to_string('website/runes/rune_card_compressed.html', {'rune': Rune.objects.get(
            id=obj_id), 'deep': True, })  # return JSON/Dict like during Desktop Upload
    elif obj_type == 'artifact':
        html = render_to_string('website/artifacts/artifact_card_compressed.html', {'artifact': Artifact.objects.get(
            id=obj_id), 'deep': True, })  # return JSON/Dict like during Desktop Upload
