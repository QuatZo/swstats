    ...
    def create(self, request):
        if request.data:
            for family in request.data:
                obj, created = MonsterFamily.objects.update_or_create(
                    id=family['id'], defaults=family, )
    ...
    def create(self, request):
        if request.data:
            for source in request.data:
                obj, created = MonsterSource.objects.update_or_create(
                    id=source['id'], defaults=source, )
            return HttpResponse(status=status.HTTP_201_CREATED)
    ...
    def create(self, request):
        if request.data:
            for base in request.data:
                ...
                base['id'] = str(base['id'])
                monster_base['family'] = MonsterFamily.objects.get(
                    id=int(base['id'][:-2]))
                ...
                obj, created = MonsterBase.objects.update_or_create(
                    id=base['id'], defaults=monster_base, )
            return HttpResponse(status=status.HTTP_201_CREATED)
    ...
    def create(self, request):
        if request.data:
            for hoh in request.data:
                ...
                ########################################
                # Monster HoH Model
                monster_hoh['monster'] = MonsterBase.objects.get(
                    id=int(hoh['id']))
                monster_hoh['date_open'] = hoh['date_open']
                monster_hoh['date_close'] = hoh['date_close']
                ########################################

                obj, created = MonsterHoh.objects.update_or_create(
                    monster=hoh['id'], defaults=monster_hoh, )
            return HttpResponse(status=status.HTTP_201_CREATED)
    ...
    def create(self, request):
        if request.data:
            for fusion in request.data:
                ...
                monster_fusion['monster'] = MonsterBase.objects.get(
                    id=int(fusion['id']))
                monster_fusion['cost'] = fusion['cost']
                ########################################

                obj, created = MonsterFusion.objects.update_or_create(
                    monster=fusion['id'], defaults=monster_fusion, )
            return HttpResponse(status=status.HTTP_201_CREATED)
    ...
    def create(self, request):
        if request.data:
            for homie in request.data:
                obj, created = HomunculusSkill.objects.update_or_create(
                    id=homie['id'], defaults=homie, )
            return HttpResponse(status=status.HTTP_201_CREATED)
    ...
    def create(self, request):
        if request.data:
            for build in request.data:
                ...
                obj, created = HomunculusBuild.objects.update_or_create(
                    depth_1=data['depth_1'], depth_2=data['depth_2'], depth_3=data['depth_3'], depth_4=data['depth_4'], depth_5=data['depth_5'], defaults=data, )
            return HttpResponse(status=status.HTTP_201_CREATED)
    ...
    def create(self, request):
        if request.data:
            if request.data['command'] == 'HubUserLogin':
                ...
                handle_dimension_hole_run_upload_task.delay(
                    request.data['response'], request.data['request'])

            elif request.data['command'] == 'GetLobbyWizardLog':
                handle_wizard_arena_upload_task.delay(
                    request.data['response'], request.data['request'])
