    ...
    def create(self, request):
        ...
        if request.data:
            if request.data["command"] == "HubUserLogin":
                ...
                logger.debug(f"Starting Battle Dungeon Result upload for {data['wizard_info']['wizard_name']}")
                dungeon = dict()
                ...
                obj.save()
                logger.debug(f"Successfuly created Battle Dungeon Result for {data['wizard_info']['wizard_name']}")
